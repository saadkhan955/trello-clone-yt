    ...
    <header>
      <div className='flex flex-col md:flex-row items-center p-5 bg-gray-500/10 rounded-b-2xl'>
        ...
        <div className='flex items-center space-x-5 flex-1 justify-end w-full'>
          ...
          <Avatar name='Saad Khan' round color='#0055D1' size='50' />
        </div>
      </div>
      <div className='flex items-center justify-center px-5 md:py-5'>
        <p className='flex items-center text-sm font-light pr-5 p-5 py-2 shadow-xl rounded-xl w-fit bg-white italic max-w-3xl text-[#0055D1]'>
          <UserCircleIcon className={`inline-block h10 w-10 mr-1 text-[#0055D1] ${loading && "animate-spin"}`} />
          {suggestion && !loading ? suggestion : 'GPT is Summarizing your task for the day'}
        </p>
      </div>
    </header>
  )
